...
from amazing.amazing.isaaclab.isaaclab.envs import ManagerBasedConstraintRLEnv
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        if hasattr(self, "policy_state_handler"):
            policy_obs = self.policy_state_handler.update(
                obs_dict["stack_policy"], obs_dict["none_stack_policy"], reset_mask=reset_mask
            )
            obs_dict["policy"] = policy_obs        

        if hasattr(self, "critic_state_handler"):
            critic_obs = self.critic_state_handler.update(
                obs_dict["stack_critic"], obs_dict["none_stack_critic"], reset_mask=reset_mask
            )
            obs_dict["critic"] = critic_obs
