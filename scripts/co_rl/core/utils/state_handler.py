...
class StateHandler:
    ...
    def update(
        ...
        Args:
            ...
            reset_mask (torch.Tensor | None): shape (num_envs,)의 bool 마스크.
                True인 환경은 스택 히스토리를 현재 프레임으로 초기화합니다.
        ...
        if reset_mask is not None:
            reset_mask = reset_mask.to(dtype=torch.bool).view(-1)
            if bool(reset_mask.any()):
                for idx in range(self.total_frames):
                    self.stack_buffer[idx][reset_mask] = stack_obs[reset_mask]
