...
class AmazingRewardsCfg():
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            # no wheel
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["FR1_1", "FL1_1", "FR2_1", "FL2_1", "SR_1", "SL_1", "base_link", "T1_1", "T2_1", "T3_1", "T4_1", "T5_1", "T6_1"]),
            "threshold": 1.0,
        },
    )
    ...
    joint_align_l1 = RewTerm(
        func=mdp.joint_align_l1,
        weight=-0.2,  # default: -0.5
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_thigh", ".*_calf"])},
    )
    flat_orientation_l2 = RewTerm(func=mdp.flat_orientation_l2, weight=-50.0)
    base_adaptive_height = RewTerm(
        func=mdp.base_height_adaptive_l2,
        weight=-0.5,
        params={
            "target_height": 0.70,
            "asset_cfg": SceneEntityCfg("robot", body_names="base_link"),
            # "sensor_cfg": SceneEntityCfg("height_scanner"),  # flat terrain
        },
    )
    ...
    base_height = RewTerm(
        func=mdp.base_height_l2, weight=-10, params={"target_height": 0.3}
    )
    is_alive = RewTerm(func=mdp.is_alive, weight=50.0)
